...
The controls for this basic Shiny version take up around 640 pixels of vertical space:

![](images/vignette_2.jpg){width="450"}

By contrast, the `inshiny` version takes up around 210 pixels of vertical space:

![](images/vignette_3.jpg){width="450"}
...
Most of the `bslib` "bootswatch" theme presets work well with `inshiny`. There 
are just a few which have some issues: `"brite"`, `"cyborg"`, `"lumen"`, and 
`"sketchy"` have some minor display issues with some widgets, while `"materia"` 
has some odd spacing problems with buttons. The other 22 `bslib` presets and
the default `"shiny"` preset work well.
